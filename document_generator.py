    ...
    *,
    ...
    S25 stamping (all optional, keyword-only):
      ...
      template_version_id   which template version produced this. Cheap to
                            ...
                            later revision.
      source_revision       the template's revision number at generation
                            time. S36's task-register producer compares this
                            against the current in-force revision to flag a
                            document as behind — it was accepted here and
                            stamped on the `documents` row, but never forwarded
                            to the register_client_document() call below, so
                            client_documents.source_revision stayed NULL for
                            every templated document ever generated. Fixed as
                            part of S36, since that field is the whole
                            mechanism.
      ...
      brand_profile_version unused until S46; taken now so the call sites do
    ...
    # Register as a DRAFT (S27). Generating is not adopting.
    #
    # This used to mark the document current the moment it was produced, so the
    # register asserted a DPA was in force before anyone had signed it, with an
    # effective date RECOSA had invented. The client now says when it begins to
    # apply, because that is a fact only they have.
    #
    # No version number is assigned here either: three discarded drafts
    # consuming v4, v5 and v6 leave a published sequence reading v3 -> v7, and
    # a gap in a version sequence is not explainable to an auditor.
