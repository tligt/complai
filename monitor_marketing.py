...
def search_news_with_agent(query: str, api_key: str, agent_id: str) -> list[dict]:
    ...
    prompt = (
        ...
        f"SOURCE QUALITY GUIDANCE:\n"
        f"- When possible, prefer articles from the original publishing outlet "
        f"(e.g. Le Monde, Le Soir, De Tijd, La Libre, national/EU official press "
        f"pages) over wire service syndication pages (e.g. AFP, Reuters "
        f"newsroom, AP newsroom), since wire service pages are often "
        f"unreadable, paywalled, or just the homepage even when the URL loads.\n"
        f"- Still include the story even if you can only find it via a wire "
        f"service or aren't fully confident in the exact URL — accuracy of "
        f"the title/summary/date matters more than having a perfect link. "
        f"In that case, set \"url\" to an empty string \"\" rather than "
        f"guessing at a URL or using a domain's homepage as a placeholder. "
        f"A missing URL is fine; a wrong or guessed URL is not.\n\n"
        f"For each article, provide your best information even if some fields "
        f"are uncertain — always return the article rather than skipping it.\n\n"
