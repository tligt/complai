    ...
    if run_review and uploaded_review:
        ...
        if not doc_text.strip():
            ...
            def make_review_pdf():
                ...
                story.append(Paragraph(f"Generated by RECOSA · {date.today().strftime('%d %B %Y')}", S("d", fontSize=9, textColor=MGRAY, leading=12, spaceAfter=16)))
                ...
                story.append(HRFlowable(width="100%", thickness=1, color=HexColor("#D3D1C7")))
                story.append(Paragraph("Generated by RECOSA · complai.be · Review with a qualified legal professional before use.", S("ft", fontSize=7, textColor=MGRAY, leading=10, spaceAfter=0)))
                doc_r.build(story)
                return buf.getvalue()
            ...
            st.download_button(
                ...
                data=review_pdf,
                file_name=f"RECOSA_review_{doc_type_review}_{company_name.replace(' ','_')}.pdf",
            ...
            if client_id:
                ...
                if not st.session_state.get(save_key):
                    ...
                    path = f"{user_id}/{client_id}/RECOSA_{doc_type_review}_{ts}_{safe}"
                    stored = upload_file(
...
with tab2:
    ...
    for doc_type in doc_types_ordered:
        ...
        if current:
            n_provided += 1
            col_s.markdown("✅")
            src = "RECOSA" if current.get("source") == "complai_generated" else "Uploaded"
        ...
        if new_file:
            ...
            if text.strip():
                ...
                if client_id:
                    ...
                    path = f"{user_id}/{client_id}/RECOSA_{doc_type}_{ts2}_{safe}"
    ...
    if run_full:
        ...
        st.download_button(
            ...
            data=pdf_bytes,
            file_name=f"RECOSA_gap_{company.replace(' ','_')}.pdf",
            mime="application/pdf",
            type="primary",
            use_container_width=True,
        )
