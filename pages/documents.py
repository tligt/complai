...
def _send_email(to: str, subject: str, body: str, file_bytes: bytes, filename: str) -> bool:
    ...
    try:
        r = _req.post(
            ...
            json={
                "sender": {"name": _os.environ.get("BREVO_FROM_NAME","RECOSA"), "email": _os.environ.get("BREVO_FROM_EMAIL","audit@complai.be")},
                "to": [{"email": to}],
                "subject": subject,
                "htmlContent": f"<p>{body}</p><p>Generated by <strong>RECOSA</strong> · complai.be</p>",
                    ...
                    if fpath:
                        try:
                            ...
                            ok = _send_email(
                                ...
                                body=f"Please find attached the {lbl} generated by RECOSA for {company}.",
