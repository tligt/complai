...
**Last updated:** 18 September 2026 (S33 closed)
...
2. **Record the decision and the rejected alternative.** "We use Postgres for
   ...
   constant *because* the S43 scanner needs to query it" is what stops a future
   reader undoing it.
...
Solo-founded, bootstrapped, pre-beta. The beta gate is **S33** (GDPR data
...
S48 document workflow comments can reuse it. Content-free Brevo reply
...
suggested activities), `vendor_domain_patterns` (empty until S43),
...
Five letter-suffixed sprints and two reorderings had left the numbers telling a
different story from the priorities. **Delivered sprints keep their numbers** —
they appear in commit messages, template `sprint` fields and change notes that
cannot be rewritten, so S26A, S26C, S28, S29, S29A and S30 are unchanged.

Everything unshipped was renumbered from S31. Relative order was preserved
except for three deliberate changes:

| was | now | why |
|---|---|---|
| S32A | S31 | Infrastructure was buried behind a letter suffix; it precedes the gate |
| S31 | S35 | Chunk allocation is chat ranking quality, not a blocker (D-70) |
| S28A | S54 | The AI deployer pack needs the AI Act inventory to describe |
| S49 + S54 | S51 | Merged — both touch the same admin pages, and two sprints editing one page in sequence is how one silently reverts the other |
...
What replaces it, from the 8 Sept diagnostic and a survey of current RAG
architectures (10 Sept): **the problem is narrower than "our RAG needs
upgrading", and three cheap things beat one expensive one.**

Retrieval answered from the right regulation in 7 of 9 queries. Both failures
were NIS2 questions phrased the way a client would ask them, and both returned
*defensible* answers from another regulation — GDPR Art. 33 genuinely does
answer "do I have to tell anyone if we get hacked".

In priority order, each measured before the next is built:

1. **Metadata filtering from the client profile.** The cheapest item and half
   built: `retrieve(regulations=[...])` exists (D-70) and chat passes nothing.
   RECOSA knows each client's applicable regulations, entity class and sector.
   A NIS2 chunk should not reach a client not subject to NIS2.
2. **Query decomposition for multi-regulation questions.** The one that fixes
   both observed failures. "Do I have to tell anyone if we get hacked" *is* two
   questions — Art. 33 and Art. 23 — and answering both is correct rather than
   a workaround. It is what the S29A breach procedure already does in document
   form.
3. **Hybrid dense + BM25.** Legal text is full of exact tokens — "Annex III",
   "Art. 21(2)(d)", "essential entity" — that embeddings blur. Would not have
   fixed the two misses; helps a different class.

**Considered and rejected for now**, with reasons, so they are not
re-proposed:

- *Self-RAG / Corrective RAG* — a grading loop and re-query per question. For a
  few hundred questions a day against a retriever already at 7/9, that is
  latency and Mistral spend for an unmeasured gain.
- *Graph RAG* — regulations genuinely are a graph (articles → recitals →
  guidelines, obligations cross-referencing each other) and this is the most
  interesting of the rejected options. Building and maintaining one over four
  regulations is a sprint on its own for a benefit not yet measured.
- *Agentic / Multi-Agent RAG* — solving team-scale research problems RECOSA
  does not have.

*Source note:* the taxonomy came from a Medium survey of seven RAG
architectures. Useful for naming things, thin on benchmarks and on when the
extra cost pays for itself. **Its own conclusion is the usable part — pick the
architecture that fits the problem — and ours is narrower than the article's
framing suggests.**
...
`source_revision` is the S36 hook: when a template's source revision bumps
...
- The S43 scanner resolves domains to vendors **by query**, against a table
  growing to hundreds of rows.
- Labels need per-language translation; codes do not.
- S47 Enterprise taxonomies are per-workspace **by definition**, which a module
...
7. **Cookies recorded at vendor level** until the S43 scanner supplies names
   and durations.
...
went wrong the first time. No selector until S32 provides a profile page, so
...
### D-90 — S33 ships two tiers, not all four named in the commercial model
*S33.*
...
data: inventing structure ahead of the thing that needs it.
...
### D-91 — Downgrade is guarded at the write; self-serve upgrade is not guarded at all
*S33.*

`set_user_tier()` refuses Advisory → Professional while the account holds
more than one client, checked inside `database.py` itself and not only
disabled in a page — the same reasoning as D-62, the status guard lives in
the store. Any future caller, not only the two pages built this sprint, is
bound by it.

Upgrade carries no such check: Professional's shape is a strict subset of
what Advisory allows, so gaining room can never invalidate anything already
recorded. It is exposed as a self-serve button on the Profile page,
deliberately — this is a revenue action, and every point of friction on it
is a lost upgrade.

Downgrade stays admin/support-only, for two independent reasons:

1. **Business** — a downgrade should be a conversation, not a button. Easy
   self-serve downgrade on a paid feature is a discount nobody configured.
2. **Technical** — Professional has no client selector anywhere in the app;
   every single-client page assumes exactly one, via `.single()` or
   `clients[0]`. An account with several clients downgraded silently would
...
New shared module `tier_gates.py`. A control blocked by tier is not hidden
and not disabled — it stays on the page, and clicking it opens an
`@st.dialog` naming the limit and linking to the upgrade path, instead of
performing the gated action.

*Rejected — disabling the control.* A greyed-out button with no explanation
reads as broken, not as "upgrade to use this". The control has to stay
reachable, and clickable, for the upsell to ever be seen — the moment
someone hits a limit is the moment they are most likely to act on it, and a
disabled control throws that moment away.

First gate: `client_limit_reached()`, applied at both existing "New client"
entry points — the Chat sidebar expander and the Profile page. Built as a
pattern deliberately, not a one-off: the next tier-gated feature is a new
check function plus a call to the existing `upsell_dialog()`, nothing about
the mechanism changes.

### D-93 — `pages_admin/users.py` is the only writer of `profiles.role`
*S33.*

Every admin account until now was made one by hand, directly in the
database — there was no other way, and no code path had ever set the
column. `set_user_role()` is now that path, and the only one.

The page disables the role control when the selected account is the
logged-in admin's own. There is exactly one admin account in the database
today, and a self-demotion here has no in-code way back to restore it.

### D-94 — Password change goes through `auth.py`'s session client, not `database.py`'s
*S33.*

`client.auth.update_user()` requires a full Supabase auth session, set via
`client.auth.set_session()`. `database.py`'s own `get_supabase()` only calls
`client.postgrest.auth(token)` — a bearer token for RLS, not an auth
session — and raises `AuthSessionMissingError` if used for this, confirmed
against the installed `supabase_auth` source rather than assumed.

`change_password()` lives in `auth.py` for exactly that reason, against
`auth.py`'s own session-bearing client. Recorded because the two modules'
...
considerably more than it was designed to. **That is the argument for S32**,
...
*Revised 18 September 2026 (S33 closed).* **This is the first revision
actually committed to `docs/SPRINT_LOG.md` in the `complai` repository.**
The 4 September revision above was maintained locally and diverged from the
copies previously committed at `SPRINT_LOG.md` and `scope/SPRINT_LOG.md`,
both of which were still frozen at the 4 September (S27-closed) text with
none of the S28–S30 or S33 history in them. Those two stale copies are
superseded by this file and should be treated as historical, not current.
