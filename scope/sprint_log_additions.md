# SPRINT_LOG additions — sessions of 3–4 September 2026

Everything below is proposed for `docs/SPRINT_LOG.md`. Renumber D-codes if S26A
already consumed some. This is the complete record of the session and
supersedes every interim file produced during it (`backlog_additions.md`,
`s26c_log_entry.md`, `s55_s56.md`, `s27_scope.md`,
`s41_scope_additions.md`) — all are merged here.

**Shipped:** S26A closed. S26C opened and largely built. S27 built bar the
auditor page.
**Scoped:** S27 (scope lock), S41 (additions).
**Registered today:** S28A, S50–S56, task register (unnumbered), plus scope
notes on S30, S40 and the vendor risk register.

## Contents

1. S26A — closed
2. S26C — multilingual text and structured retention
3. S27 — document register (scope lock + build record, D-57 to D-62)
4. S41 — billing (scope additions)
5. New sprints and scope notes (S28A, S50–S54, task register)
6. S55, S56 and S57
7. Open checks
8. Deferred
9. Constraints and learnings
10. External findings
11. Competitor reference data

---

# S26A — DPA (Art. 28 processor clauses) — CLOSED

Render side wired, gated, and verified against generated output.

## What landed

- `build_dpa_block_context()` and `apply_dpa_values()` dispatched from
  `build_block_context()` / `build_values()` in `template_store.py`.
- `DPA_BLOCK_RENDERERS` merged into `DEFAULT_BLOCK_RENDERERS` **from
  `template_store`, not `template_renderer`** — `template_dpa` imports `Block`
  and `FieldSpec` from the renderer at module level, so registering there would
  close the import cycle. Same dict object, so the mutation is visible to
  `generate()`.
- `"dpa"` added to `TEMPLATE_DOC_TYPES`.
- DPA removed from the offer list when the client has no processor-role
  activity — same fact that already removes `ropa_processor`. Clause 1(c)
  attaches these Clauses to the processing Annex II specifies; with no
  processor-role activity the contract would describe nothing.
- Annex III gate naming the offending activities.
- D-44 columns on `clients`: `sub_processor_notice_days` (CHECK 1–365),
  `dpa_assistance_text`, `dpa_breach_elements_text`.

## Verified output

DPA generated for creagent SRL in French, 3 September. Schedule 1 listed SD
Worx only, with Google Ireland correctly absent despite being in the inventory —
the processor-role scoping held, which was the check that mattered most. Notice
period rendered "au moins 30 jours". Annex IV emitted with its explanation of
why it is empty. Annex I left blank for the controller (D-41).
...
## Carried out of S26A — still open

**Clause 7.5 sensitive-data safeguards duplicate Annex III.** The generated DPA
showed health data in Annex II with a safeguards cell byte-identical to Annex
III part 1, because `build_annex_ii_rows` reads the same `security_measures`
field. Clause 7.5 requires *specific* limitations or *additional* safeguards
where sensitive data is processed; as rendered, the contract asserts nothing
additional is done. Needs either a `sensitive_safeguards` field on the activity
or a cell that states plainly that the general measures apply.

**`DPA_FIELDS` has no `legal_form`.** Annex I identified the party as
"creagent" where the RoPA templates carry `{{#if:has_legal_form}}`. Cosmetic in
a register; in a signed contract the legal form is part of who is bound.

**`_BREACH_ELEMENTS_DEFAULT` wording slip.** Refers to "the systems listed in
Schedule 1" / "les systèmes énumérés à l'annexe technique 1", but Schedule 1
lists sub-processors. One line, both languages.

**Clause headings run into following text** in the generated PDF ("Clause 2
Invariabilité des clausesa) Les parties"). Either missing blank lines in the
OJ-derived markdown or a PDF text-extraction artefact — check the DOCX to tell
which. If real it lives in `extract_oj_pdf.py` and affects the D-42 diff.

**`sort_order=40`** in `template_seed_dpa.py` still carries a TODO. RoPA
controller is 20, processor 21.

**D-42 stands:** the template ships marked unreviewed until counsel has seen
the diff against the OJ.

---

# S26C — multilingual activity text and structured retention

**Status:** built, partially verified. Migrations applied, vocabulary seeded.
**Position:** after S26A, before S27 (D-46).

## Why

A DPA generated in French carried, inside Annex II: purpose "Calculating and
paying salaries, and meeting social security and tax reporting obligations",
duration "5 years (social documents, Belgian law)". Both English, in a French
contract, because they are client free text stored once and rendered into every
language. The same defect affects both Art. 30 registers and — found later —
the published Cookie Policy. The DPA only made it visible.

## Shipped

**Schema.** `processing_activities` gains `name_i18n`, `purpose_i18n`,
`translation_status` (all JSONB), `retention_value`, `retention_unit`,
`retention_basis_code`, and the three archive-phase equivalents. `systems`
gains `purpose_i18n` and `translation_status`.
`vendor_catalogue_activities` gains `retention_value` / `retention_unit`.
Legacy columns retained throughout, append-only.

**Vocabulary.** 14 `retention_basis` codes and 4 `retention_unit` codes in
`reference_values`, EN and FR labels and notes, jurisdiction and `statutory`
flags in `metadata`.

**Rendering.** `format_retention()`, `build_retention_cells()`, `_i18n()`,
`_is_unreviewed()` in `template_store.py`; both registers, the Cookie Policy
vendor table and DPA Annex II now resolve per language and render both
retention phases.

**Catalogue carry-through.** `seed_rows_for()` writes every language the
catalogue holds instead of `pick()`ing one — a Belgian client seeding Microsoft
365 in English was permanently losing the French purpose the catalogue already
had.

**Translation.** New `translate.py` — authoring-time drafts under human review,
Mistral small, `temperature: 0`, token usage logged as feature `"translate"`,
returns `{}` on any failure and never raises.

**Form.** `pages/inventory.py` rewritten: one column per document language,
structured two-phase retention controls, translation gaps in the summary table.

## Not shipped

**Systems grid purpose.** `st.data_editor` has no place to review a
per-language draft — the same reason activities use list-plus-detail — so
wiring translation into it would produce drafts with nowhere to confirm them.
Rendering already reads `purpose_i18n` with fallback, so Cookie Policies are
correct once data exists; only the input path is missing. Three options:
give systems a detail form; accept single-language system purposes; or drop
`purpose` from the Cookie Policy vendor table and rely on the vendor category,
which already renders correctly in every language and is closer to what Art.
30(1)(d) asks for. **Undecided.**

**End-to-end translation verification.** The drafting path has not yet been
seen working against a live Mistral call.

## Decisions

**D-46 — S26C lands before S27.** S27 stamps and registers documents; one
...
**D-47 — client text is JSONB; catalogue text stays suffix columns.** The
...
**D-48 — retention is not backfilled.** Parsing "5 years (social documents,
...
**D-49 — retention has two phases, not one period.** CNIL separates *base
...
**D-51 — reference notes stay generic; citations deferred.** A cited note is
...
**D-52 — `vendor_determined` is not a retention basis.** Who set a period is
...
**D-53 — translation is generated on save, and failure is not an error.** A
...
**D-56 — no privileged source language in the activity form.** One column per
...
# S27 — Document register — BUILD RECORD

Appends to the S27 scope lock already in the log. Everything below was built
and verified in the session of 4 September 2026.

## Built

**Migration.** `client_documents` extended, not replaced — four consumers
already depend on it (`document_generator.py` writes on every generation,
`pages/gap.py` writes on upload and reads for scoring, `pages/dashboard.py`
reads, `gap_assessment.py` consumes the shape). A second register beside it
would have been the divergence this codebase names as its anti-pattern.

Added: `language` in the key, `status`, `adopted_at`, `effective_from`,
`published_at`, `superseded_on`, `superseded_by`, `document_id`,
`template_version_id`, `source_revision`, `retain_until`, `legal_hold`,
`hold_reason`. `is_current` rebuilt as a generated column. Partial unique index
on one in-force row per key; unique version per key. Status transition trigger.

**Store layer** (`database.py`): `adopt_client_document`,
`archive_client_document`, `delete_draft_document`, `set_legal_hold`,
`set_document_comment`, `get_register_status`, `get_latest_draft`,
`get_template_languages`, `delete_file`.

**Pure logic** (`register.py`, new): `document_status`, `status_note`,
`coverage`, `supersession_chain`.

**UI:** adoption and discard controls in the document history; register status
and version on every history row; per-language status and gap attribution on
the dashboard checklist.

## Not built

- The auditor-facing register page.
- `coverage()` is written and tested but NOT wired into the dashboard score.
  The existing calculation still counts documents the old way; changing it
  moves a client-visible number and deserves its own look.
- Export of the register as a document (deliberately deferred at scope-lock).
...
## Constraint — Streamlit

**A button nested inside a button-gated block can never fire.**

The adoption control was first placed under each generated document, which is
where a client would look for it. It could not work there. Streamlit reruns the
whole script on every interaction; the generation blocks are gated on
`generate`, which is a button and is True for exactly one run. Clicking "Put in
force" starts a NEW run in which `generate` is False, so the branch never
executes, the button is never evaluated, and nothing happens — the panel simply
disappears.

Same family as the existing note about widgets inside `st.form`.

Adoption therefore lives in the document history, which renders on every run —
and that turned out to be the better home anyway: the client adopts a version
from a list of versions rather than from whichever one they generated last.

**Also:** do not nest `st.columns` inside an already-narrow column. Two action
buttons placed inside `c0` (the narrowest of six) left the second with nowhere
to render, and it silently did not appear. Laid out at container level instead.

---

## The `if use_template:` pattern, third and fourth instances

*(A fifth followed — declaring an `st.Page` without registering it in
`st.navigation` — and a sixth of a different kind, D-65, where a column's
meaning depends on a sibling column. See the carried-forward list.)*

Recorded earlier this week with two instances. Two more today:

3. **The adoption panel was added to the LLM generation branch only**, so every
   templated document — cookie policy, both registers, the DPA — silently had
   no way to be put in force.
4. **A blanket replace of `.order("created_at")` → `"uploaded_at"`** in
   `database.py` hit all seven call sites; five belonged to `audits`,
   `profiles`, `usage_logs`, `answer_feedback` and `marketing_updates`, which
   legitimately have that column.

Both are the same shape: **a change made where the name matched rather than
where the behaviour lives.** For (3) the fix was to define the function once,
above both branches; for (4), to resolve each call site's table before editing.

---

## Carried forward

- **Auditor page.** Full chain per document, effective-date ranges, who adopted
  and when, `source` distinguishing what RECOSA vouches for from what the
  client uploaded, retention dates, legal-hold toggle.
  `register.supersession_chain()` exists for it.
- **Wire `coverage()` into the dashboard score.** Moves a client-visible
  number.
- **Fix the `pages/documents.py` template-availability message** per D-60.
- **`adopt_client_document` is not atomic.** PostgREST has no cross-call
  transaction, so a failure between superseding the predecessor and promoting
  the successor leaves a key with no in-force document — visible, wrong, fixed
  by re-adopting. A Postgres function is the right answer if this ever runs
  concurrently for one client.
- **`delete_file()` now exists** in `database.py`; nothing deleted from storage
  before. S33's deletion work needs the same helper at much wider scope.
- **Legal hold is recorded but not enforced.** `retain_until` and `legal_hold`
  are stored and nothing reads either — no sweep exists. The flag still carries
  meaning (an auditor seeing a version under hold learns the client manages
  retention deliberately), but S33's sweep MUST check `legal_hold` before
  deleting anything, or the mechanism is decorative and the failure mode is
  destroying evidence during a live proceeding.
- **`hold_set_on` timestamp is missing.** S27 stores the flag and the reason
  but not when the hold was set, so S57's 30-day staleness check has no clock
  to run from. Add it before any hold is set in anger — the information cannot
  be reconstructed later.
- **Page-in-`pages/` name collisions.** `pages/register.py` importing the root
  `register.py` resolved to itself: Streamlit puts a page's own directory on
  `sys.path`. Silent until runtime, and the traceback points at the import line
  rather than at the collision. The page was renamed to
  `pages/compliance_record.py`. A module in `pages/` must not share a name with
  a root module it imports.
- **Declaring an `st.Page` does nothing on its own.** It must also appear in
  the `st.navigation` dict AND in `PAGE_CONTEXT` in `app.py`. A page missing
  from the dict fails silently — no error, no warning, just an absent link.
  Fifth instance of the declaration-and-registration-in-two-places shape.
- **Historical version numbers are not dense.** Rows backfilled from the old
  per-generation numbering keep their original numbers. Cosmetic on existing
  rows, correct from adoption onwards.
## S27 — additions after the build record

Same sprint, later in the session. Four things, three of which were bugs found
by looking at the screen rather than by testing.
...
### Activity log rebuilt as a table

Prose lines became a sortable table with filters and CSV export. Not cosmetic:
the log is evidence, and evidence gets handed over. A client asked for their
activity history by an auditor should not be screenshotting a page, and an
auditor arrives with a question — what happened to the DPA, who released that
hold — rather than to read 200 rows in order.

Also: readable labels throughout. `audit_log` stores internal codes, and four
summary writers in `database.py` were emitting raw `doc_type` — "dpa v2"
rather than "Data Processing Agreement v2". Fixed at all four via a lazily
imported `_doc_label()`, since `database.py` does not import `obligations` at
module level and adding one for a display concern risks a cycle. Existing rows
keep their old summaries; audit entries are immutable.

And "held 0 day(s)" — accurate, and reads like a defect. Now "the same day".

### Carried forward

- `DISPLAY_TZ_NAME` becomes a per-client setting when the first non-Belgian
  client arrives.
- S57's stale-hold check now has its clock (`hold_set_on`) and its trail
  (`hold_set` / `hold_released` events). Buildable whenever it comes up.
- The compliance record shows the reason a hold is ACTIVE. The reason one was
  RELEASED exists only in the audit event, because `hold_reason` is cleared
  from the row — a released hold is not a property of the document but a
  decision someone made. The activity log is the only place it can appear.
---
...
branch.** The templated preview block in `pages/documents.py` was written in
S25 when `cookie_policy` was the only templated document. It described the
cookie vendor list and called `_load_vendor_rows` unconditionally. S26 added
two registers and S26A a DPA; all three inherited a caption about systems
"marked as setting cookies" above a list of cookie vendors, on documents that
read neither. It does not fail loudly — it describes whichever document was
there first.

**Three instances of the same shape in one session.** The legacy `dpa` intake,
the vendor preview block, and `seed_from_catalogue`'s guidance note (which
tested `retention_period` — once the catalogue stopped writing that field, the
branch would have fired for *every* seeded activity, appending the guidance note
to rows that already had a period). All three were correct until the thing they
read changed underneath them. **A condition that reads a field is a dependency
on that field continuing to be written.**

**`save_activity` sends every allowlisted column, `None` where absent.**
`{c: row.get(c) for c in ACTIVITY_EDITABLE}` is fine for nullable columns and
fatal for `NOT NULL` JSONB ones — every save would have failed on a constraint
violation before the form was even rewritten. `diff_by_id` guards with
`if c in edited_df.columns`; `save_activity` does not. Coalesced rather than
dropped, because on an UPDATE omitting a key leaves the stored value alone and
a caller clearing a translation would have no way to say so.

**Structured data can be half-set in ways prose cannot.** The seed self-check
gained three rules: a value with no unit, a unit with no value, an unrecognised
unit. A half-set period renders as a bare number or vanishes, and the register
is where that shows up.

**A UI that pre-fills a box from a different language is a data-integrity bug,
not a cosmetic one.** See D-56.
...
**S27 is complete.** Register, adoption with client-supplied effective dates,
supersession chains, drafts and discard, legal hold with audit trail, the
auditor-facing compliance record page, and the dashboard reading the same
logic through `register.py`.

**Applied to the database:** `migration_s26a.sql`, `migration_s26c.sql`,
`migration_s26c_systems.sql`, `migration_s26c_catalogue.sql`,
`reference_values_type_valid` extended to 17 value_types, `seed_s26c_full.sql`,
`migration_s27.sql`, `migration_s27_hold.sql`.

**Committed:** `app.py`, `database.py`, `document_generator.py`,
`template_store.py`, `template_dpa.py`, `inventory.py` (root),
`inventory_store.py`, `inventory_seed.py`, `pages/documents.py`,
`pages/inventory.py`, `pages/gap.py`, `pages/dashboard.py`,
`pages/activity.py`, `pages/compliance_record.py`, and two new root modules —
`translate.py` and `register.py`.

**Verified working.** DPA generation with correct processor-role scoping.
S27 draft → adopt → supersede → hold → release, with the audit trail, the
compliance record page, and Brussels-time display.

**Unverified.** The S26C translation drafting path has still not been seen
working against a live Mistral call. If `MISTRAL_API_KEY` is absent from the
Streamlit Cloud environment, or the model returns something the JSON extractor
rejects, it fails silently by design — an empty column and no error. First
thing to check.

**Next, in rough order:** S28 (Privacy Policy + AI Transparency Notice), with
the open question of whether S31 should come first — S28, S29 and S30 all
produce documents with LLM inserts, and authoring templates against a retrieval
layer already suspected of being wrong means reviewing the output twice.

**Reminder for whoever picks this up.** No decision in this file exists
anywhere else. Conversation does not persist; `docs/SPRINT_LOG.md` is the only
record. D-55 is this session's own evidence for that — a correct decision was
almost silently reversed, and what caught it was D-40's recorded *rejected
alternative*, not its conclusion.